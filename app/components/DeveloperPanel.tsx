...
const SPECIAL_PIXEL_IMAGES: Record<string, string> = {
  ...
  eriksenHeartAttack:  "/images/eriksen-pixel.png",
  dopingBan:           "/images/doping-ban-pixel.png",
  girlsMagnet:         "/images/girls-magnet-pixel.png",
  racistAttack:        "/images/racism-attack-pixel.png",
  clubLegend:          "/images/club-legend-pixel.png",
};
...
const DEV_EVENTS: { id: DevEventId | string; label: string; type: "positive" | "negative" | "special" }[] = [
  ...
  { id: "eriksenHeartAttack", label: "Heart Attack",         type: "negative" },
  { id: "dopingBan",          label: "Doping Ban",           type: "negative" },
  { id: "racistAttack",       label: "Racist Attack",        type: "negative" },
  ...
  { id: "bobPaisleyDisaster", label: "Bob Paisley Disaster", type: "special"  },
  { id: "dreamSeason",        label: "Dream Season",         type: "special"  },
  { id: "lockerRoomDrama",    label: "Locker Room Drama",    type: "special"  },
  { id: "clubLegend",         label: "Club Legend",          type: "special"  },
...
export default function DeveloperPanel({ onTrigger, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="font-black text-purple-300 text-base">Developer Panel</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 bg-red-700 hover:bg-red-600 text-white text-sm font-bold transition-all flex items-center justify-center"
          >
            ×
          </button>
        </div>
      ...
      <div className="p-4 max-h-[70vh] overflow-y-auto">
        {(["positive", "negative", "special"] as const).map((type) => (
          <div key={type} className="mb-3">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {DEV_EVENTS.filter((e) => e.type === type).map((event) => {
                const imgSrc = EVENT_PIXEL_IMAGES[event.id as DevEventId] || SPECIAL_PIXEL_IMAGES[event.id];
                return (
                  <button
                    key={event.id}
